...
$operationalCss=(string)file_get_contents($operationalCssFile);$operationalJs=(string)file_get_contents($operationalJsFile);$summary=(string)file_get_contents($summaryFile);
foreach(['viewport','cockpit.css','cockpit.js','cockpit-operational.css','cockpit-operational.js','Casos','Revisões','case-list','case-detail'] as $needle)cuAssert(str_contains($page,$needle),'Cockpit page missing '.$needle);
...
foreach(['operationalBucket','if(state.view===\'cases\')loadCases()'] as $needle)cuAssert(str_contains($operationalJs,$needle),'Operational cockpit startup missing '.$needle);
cuAssert(str_contains($summary,'AmazonReturnsSummary'),'Dedicated summary module must own top dashboard rendering.');
foreach(['Nenhuma ação sua é necessária','Sua decisão é necessária','Aguardando resposta da Amazon','Aguardando crédito da Amazon','Providência automática atrasada','Data do pedido','Reembolso concedido ao cliente','Débito na conta da loja','NF de venda','Última verificação','Próxima providência','Ver histórico completo','Pedido sincronizado','Movimentação financeira identificada'] as $needle)cuAssert(str_contains($operationalJs,$needle),'Operational copy missing '.$needle);
...
cuAssert(!is_file($root.'/admin/amazon-returns/assets/ux-polish.js'),'Legacy DOM mutation patch must be removed.');
cuAssert(!is_file($root.'/admin/amazon-returns/assets/operator-language.js'),'Legacy language override must be removed.');
cuAssert(!is_file($root.'/admin/amazon-returns/assets/cockpit-operational-bootstrap.js'),'Legacy operational bootstrap must be removed.');
...
cuAssert(str_contains($js,"if(raw==='SESSION_EXPIRED')return raw;"),'Session-expired sentinel must survive friendly error normalization.');
...
cuAssert(!str_contains($page,'operator-language.js'),'Operator language must be owned by cockpit.js, not a runtime override.');
cuAssert(!str_contains($page,'ux-polish.js'),'Review ordering must be deterministic without a DOM mutation patch.');
cuAssert(!str_contains($page,'cockpit-operational-bootstrap.js'),'Operational behavior must not depend on a runtime monkey-patch bootstrap.');
cuAssert(str_contains($js,'function recommendationExplanation('),'Recommendation language must live in cockpit.js.');
cuAssert(str_contains($operationalJs,'function syncOperationalChrome('),'Operational chrome ownership must live in cockpit-operational.js.');
