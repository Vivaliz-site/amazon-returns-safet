...
final class SvAmazonReviewService
{
    ...
    { $mode=(string)($d['decision_mode']??'');if(!in_array($mode,['APPROVED','EDITED_APPROVED','REJECTED','WAIT','EXCEPTION'],true))throw new InvalidArgumentException('Invalid decision mode.');$action=(string)($d['final_action']??'');if($action==='')throw new InvalidArgumentException('Final action required.');$params=is_array($d['parameters']??null)?$d['parameters']:[];if($mode!=='EXCEPTION'&&isset($params['date'])&&!isset($params['date_binding']))throw new InvalidArgumentException('Reusable decisions cannot contain literal custom dates.');$d['actor']=$actor;$d['source_version']=(string)($d['source_version']??'cockpit-v1');$d['parameters']=$params;return $d; }
    private function ruleDefinition(array $review,array $d):array
    { $ctx=$review['context']??[];$match=is_array($ctx['signature']??null)?$ctx['signature']:[];if(($match['material_conflict']??false)===true)throw new RuntimeException('Material conflict cannot be promoted.');$effect=['action'=>$d['final_action'],'parameters'=>['date_binding'=>(string)($d['parameters']['date_binding']??'NONE')]];return ['rule_family_key'=>hash('sha256',json_encode($match,JSON_THROW_ON_ERROR|JSON_UNESCAPED_SLASHES)),'match'=>$match,'effect'=>$effect,'source_review_id'=>(int)$review['id'],'specificity'=>count($match)]; }
