...
srAssert(str_contains($calls,'release amazon_returns_safet 7 11'),'Quiescence must release orphaned read-only leases before waiting.');
srAssert(str_contains(implode("\n",$output),'worker_quiesce_released_orphaned_reads=1'),'Recovery must be observable.');
...
srAssert(str_contains($source,"status='PENDING'"),'Orphaned read jobs must return to PENDING.');
...
srAssert(str_contains($source,"last_error='WORKER_INACTIVE_DURING_QUIESCE'"),'Recovery must record that the browser worker was inactive.');
@unlink($counts);
@unlink($log);
@rmdir($tmp);
