...
def migrate(config_path: Path, *, state_root: Path, controller_state: Path) -> bool:
    ...
    if payload.get("auto_dispatch") is True:
        raise StateLayoutMigrationError("refusing legacy ledger migration while auto_dispatch=true")
    ...
    if not legacy.exists():
        ...
        return True
    sources = [legacy, Path(str(legacy) + "-wal"), Path(str(legacy) + "-shm")]
    moves: list[tuple[Path, Path]] = []
    for source in sources:
        if not source.exists():
            continue
        destination = controller_state / source.name
