...
$w=(string)file_get_contents($wrapper);$b=(string)file_get_contents($broker);$u=(string)file_get_contents($unit);$deploy=(string)file_get_contents($root.'/scripts/auto-deploy.sh');
...
if(str_contains($deploy,'sudoers'))throw new RuntimeException('Deployment must not install Codex sudoers rules.');
foreach(['amazon-returns-codex-review.service','/home/ubuntu/.codex/auth.json','codex_review_broker=ready','codex-review-wrapper.sh'] as $needle)if(!str_contains($deploy,$needle))throw new RuntimeException('Auto-deploy must activate Codex broker: '.$needle);
