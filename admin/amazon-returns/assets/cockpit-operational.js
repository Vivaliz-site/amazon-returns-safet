...
function renderOperationalEvidence(c,timeline){
  ...
  const sources=[...new Set((timeline||[]).map(x=>sourceLabel(x.source)).filter(x=>x&&x!=='—'))];if(sources.length)block.append(text('p',`Fontes consultadas: ${sources.join(', ')}.`,'muted'));
...
function renderOperationalCase(data,relatedCount=null){
  ...
  const finance=sectionBlock('Valores','case-financial');appendAvailable(finance,operationalField('Valor reembolsado ao cliente',brl(c.refund_amount)),operationalField('Valor já recuperado',brl(c.reconciled_credit_amount)),operationalField('Saldo ainda a recuperar',brl(c.outstanding_amount),'emphasis'));root.append(finance);
  ...
  const reason=sectionBlock('Por que o sistema tomou esta decisão?');reason.append(text('p',reasonLabel(c.current_reason)||'A decisão foi tomada com base nos dados atuais do pedido, do reembolso, da devolução e do financeiro.'));root.append(reason);
