...
foreach (['FBA_ONSITE' => 201, 'DELIVERY_BY_AMAZON' => 202] as $program => $policyId) {
    $d59x = SvAmazonReturnPolicyEngine::evaluate(
    ...
    policyAssertSame(false, $d59x['eligible'], "{$program} must not be eligible before D+60.");
    policyAssertSame($policyId, $d59x['policy_version_id'], "{$program} must select its D+60 policy version.");

    $d60x = SvAmazonReturnPolicyEngine::evaluate(
        applicableCase(['program' => $program, 'order_at' => '2026-04-21 00:00:00']),
        new DateTimeImmutable('2026-07-01 12:00:00', new DateTimeZone('UTC'))
    );
    policyAssertSame(true, $d60x['eligible'], "{$program} must be eligible at D+60.");
}
