...
final class SvAmazonSpApiEventSink
{
    ...
    public static function persist(
        SvAmazonTenantPersistence|PDO $target,
        array $order,
        array $transactions
    ): array {
        if ($target instanceof PDO) return self::persistLegacy($target, $order, $transactions);
        return self::persistScoped($target, $order, $transactions);
    }

    /** @return array{cases:list<int>,single_item:bool,refund_event_id:?int} */
    private static function persistScoped(
    ...
    private static function persistLegacy(PDO $db, array $order, array $transactions): array
    {
        $orderId = trim((string)($order['order_id'] ?? ''));
        if ($orderId === '') throw new InvalidArgumentException('SP-API order_id is required.');
    ...
    public static function persistReturnsReportRow(
        ...
        if ($target instanceof PDO) return self::persistReturnsReportRowLegacy($target, $row, $reportId);
    ...
    private static function persistReturnsReportRowLegacy(PDO $db, array $row, string $reportId): array
    {
        $orderId = SvAmazonReturnsReportParser::orderId($row);
        $itemId = SvAmazonReturnsReportParser::orderItemId($row);
        if ($orderId === '') return ['matched'=>false,'applied'=>false];
