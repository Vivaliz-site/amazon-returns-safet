...
$browserHelper=(string)file_get_contents(__DIR__.'/../scripts/ensure-playwright-chromium.sh');
...
oprAssert(str_contains($script,'ensure-playwright-chromium.sh'),'Olist provisioner must restore a missing Chromium binary before configuring the browser service.');
oprAssert(str_contains($browserHelper,'PLAYWRIGHT_BROWSER_VERSION') && str_contains($browserHelper,'1.63.0'),'Browser bootstrap must pin the matching Chromium package version.');
