...
- D+45 operational opening remains unchanged for the current ShopVivaliz tenant.
...
Require the production verification script to compare tenant-scoped counts and hashes and to assert zero null owners and zero child/parent ownership mismatches. Keep D+45 operational opening and secret-redaction assertions.
...
Expected: PASS without changing D+45 operational opening definitions.
...
`verify-migration.sh` exports canonical ordered rows including owner IDs, computes SHA-256 hashes, compares 37 source/target current cases for the ShopVivaliz cutover, checks D+45 operational opening, checks zero null owners/mismatches, and redacts environment contents.
