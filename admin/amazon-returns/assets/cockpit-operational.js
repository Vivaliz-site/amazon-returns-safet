...
function operatorNextRelevantDate(c){const state=String(c?.state||''),action=String(c?.current_action||''),reason=String(c?.current_reason||'');if(['RECOVERED','CLOSED_LOSS','RECEIVED_OK'].includes(state))return null;const appealPending=action==='SAFE_T_APPEAL'||['SAFE_T_DENIED','SAFE_T_INFO_REQUESTED','APPEAL_REQUIRED'].includes(state);if(appealPending&&c?.appeal_deadline_at)return c.appeal_deadline_at;if(c?.next_action_at)return c.next_action_at;const openingPending=action==='SAFE_T_SUBMIT'||['SAFE_T_ELIGIBLE','SAFE_T_READY'].includes(state)||(action==='WAIT'&&reason==='NOT_YET_ELIGIBLE');if(openingPending&&c?.eligibility_at)return c.eligibility_at;return null;}
