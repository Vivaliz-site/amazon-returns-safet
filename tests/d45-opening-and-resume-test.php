...
$finance=['id'=>102,'case_id'=>77,'event_type'=>'FINANCIAL_REFRESH_CONFIRMED','source'=>'SP_API_FINANCES','occurred_at'=>'2026-09-10 11:00:00','payload'=>['refresh_complete'=>true]];
$resume=$engine->nextAction($claim,[$wait,$finance],$policy,new DateTimeImmutable('2026-09-10T12:00:00Z'));
dorSame('SAFE_T_APPEAL',$resume['action'],'resume existing claim after requested date and refreshed unpaid finance');
$again=$engine->nextAction($claim,[$wait,$finance],$policy,new DateTimeImmutable('2026-09-10T13:00:00Z'));
dorSame($resume['idempotency_key']??'first',$again['idempotency_key']??'second','one followup per request/date, not one per poll');
$paid=$claim;$paid['reconciled_credit_amount']='100.00';dorSame('WAIT',$engine->nextAction($paid,[$wait,$finance],$policy,new DateTimeImmutable('2026-09-10T12:00:00Z'))['action'],'do not reopen a financially recovered case');
...
$newDate=$engine->nextAction($claim,[$wait,$finance,$changed],$policy,new DateTimeImmutable('2026-09-10T13:00:00Z'));
