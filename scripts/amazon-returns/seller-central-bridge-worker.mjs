...
async function safeTSubreasonDiagnostics(cdp, value) {
  const expected=String(value||'').trim();
  try {
    return String(await cdp.evaluate(`(()=>{const expected=${JSON.stringify(expected)};const docs=[];const visit=d=>{if(!d||docs.includes(d))return;docs.push(d);for(const frame of d.querySelectorAll('iframe')){try{visit(frame.contentDocument)}catch{}}};visit(document);const roots=[];const addRoot=r=>{if(!r||roots.includes(r))return;roots.push(r);for(const el of r.querySelectorAll?.('*')||[]){if(el.shadowRoot)addRoot(el.shadowRoot)}};for(const d of docs)addRoot(d);const dropdowns=roots.flatMap(root=>[...(root.querySelectorAll?.('kat-dropdown')||[])]);const options=[];const seen=new Set();for(const root of roots){for(const option of root.querySelectorAll?.('kat-option,option,[role="option"]')||[]){if(seen.has(option))continue;seen.add(option);options.push(option)}}const valueOf=option=>String(option.getAttribute?.('value')??option.value??option.dataset?.value??'').trim();const values=[...new Set(options.map(valueOf).filter(v=>/^[A-Za-z0-9_.:-]{1,40}$/.test(v)))].slice(0,12);const exact=options.filter(option=>valueOf(option)===expected).length;return ['d='+dropdowns.length,'r='+roots.length,'o='+options.length,'e='+exact,'v='+values.join(',')].join(';')})()`)).slice(0,220);
  } catch {
