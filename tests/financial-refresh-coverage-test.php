...
frcSame(true,str_contains($query['sql'],"state='RECOVERED'"),'recovered cases still need reversal observation');
frcSame(true,str_contains($query['sql'],'closed_at IS NULL'),'open cases must remain in the financial rotation');
frcSame(false,str_contains($query['sql'],'closed_at IS NULL OR expected_reimbursement_amount>0'),'closed non-recovered cases must not be scanned forever');
