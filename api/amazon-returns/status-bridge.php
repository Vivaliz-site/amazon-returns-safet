...
require_once dirname(__DIR__,2).'/includes/amazon-returns/StatusBridgeService.php';
...
function sv_amz_status_service_reply(array $result): never
{
    $status=(int)($result['http_status'] ?? 200);
    unset($result['http_status']);
    sv_amz_status_reply($result,$status);
}

$expected=getenv('SELLER_CENTRAL_BRIDGE_TOKEN');
$expected=is_string($expected)?trim($expected):'';
if(!SvAmazonReturnsRemoteBridge::authorized($expected,sv_amz_status_auth_header())){
...
if((int)($_SERVER['CONTENT_LENGTH'] ?? 0)>131072){
    sv_amz_status_reply(['status'=>'PAYLOAD_TOO_LARGE'],413);
}
...
try{
    ...
    sv_amz_status_service_reply($service->acceptResult((int)$jobId,$key,$result));
}catch(Throwable $e){
    error_log('[amazon-returns-status-bridge] '.get_class($e));
