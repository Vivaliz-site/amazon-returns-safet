...
$reserved=(new SvAmazonSafeTDecisionEngine())->nextAction($case,$events,[],new DateTimeImmutable('2026-09-10T12:00:00Z'))['resume_scope'];
try{$mail=SvAmazonSafeTEmailReview::composeReply($case,$events,new DateTimeImmutable('2026-09-10T12:00:00Z'),$reserved);if($mail['thread_id']!=='original-thread')$errors[]='Resumption must stay in original thread';if($mail['in_reply_to']!=='<amazon-message@amazon.com>')$errors[]='Reply correlation must persist';}catch(Throwable $e){$errors[]='Due-date resumption must compose after verified finance: '.$e->getMessage();}
foreach(['2026-09-09T12:00:00Z','2026-09-11T12:00:00Z'] as $at){$rejected=false;try{SvAmazonSafeTEmailReview::composeReply($case,$events,new DateTimeImmutable($at),$reserved);}catch(LogicException){$rejected=true;}if(!$rejected)$errors[]='Not due or stale finance must not permit a reply at '.$at;}
$paid=$case;$paid['reconciled_credit_amount']='100.00';$rejected=false;try{SvAmazonSafeTEmailReview::composeReply($paid,$events,new DateTimeImmutable('2026-09-10T12:00:00Z'),$reserved);}catch(LogicException){$rejected=true;}if(!$rejected)$errors[]='Paid case must not reopen by email';
...
foreach([null,str_repeat('a',64)] as $wrongScope){$rejected=false;try{SvAmazonSafeTEmailReview::composeReply($case,$events,new DateTimeImmutable('2026-09-10T12:00:00Z'),$wrongScope);}catch(LogicException){$rejected=true;}if(!$rejected)$errors[]='Dated reply must bind exactly to reserved job scope, not infer a different send';}
$replacement=$wait;$replacement['id']=105;$replacement['occurred_at']='2026-09-10 11:59:00';$replacement['payload']['review_next_action_at']='2026-09-09 03:00:00';$replacement['payload']['review_excerpt']='Retorne em 09/09/2026.';
$rejected=false;try{SvAmazonSafeTEmailReview::composeReply($case,[...$events,$replacement],new DateTimeImmutable('2026-09-10T12:00:00Z'),$reserved);}catch(LogicException){$rejected=true;}if(!$rejected)$errors[]='New Amazon instruction must invalidate an older reserved reply';
