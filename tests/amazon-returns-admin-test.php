...
adAssert(str_contains($caseRepo,"$unclassified=\"("),'Unclassified gate must be explicitly grouped before operational filters.');
...
adAssert(str_contains($caseRepo,"$creditMismatch=\"c.closed_at IS NULL AND c.state<>'RECOVERED' AND $exposure<=0\""),'Credit mismatch gate must only flag fully recovered exposure that remains operationally open.');
