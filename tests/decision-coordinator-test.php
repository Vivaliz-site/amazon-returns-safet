...
$now=new DateTimeImmutable('2026-09-06T03:00:00Z');$p=new DcP;$base=new SvAmazonSafeTDecisionEngine();$enabledConfig=new SvAmazonReturnsConfig(['AMAZON_RETURNS_LEARNED_RULE_EXECUTION'=>'1']);$c=new SvAmazonDecisionCoordinator($base,$p,$enabledConfig);
...
$pShadow=new DcP;$pShadow->learnedRules->rows=$p->learnedRules->rows;$shadow=new SvAmazonDecisionCoordinator($base,$pShadow,new SvAmazonReturnsConfig(['AMAZON_RETURNS_LEARNED_RULE_EXECUTION'=>'0']));$shadowDecision=$shadow->nextAction($amb,[],$policy,$now);dcSame('BLOCKED_REVIEW',$shadowDecision['action'],'disabled learned execution stays review gated');dcSame(9,$shadowDecision['learned_rule_shadow_match']??null,'disabled learned execution reports shadow match');dcSame(0,count($pShadow->ruleApplications->rows),'shadow no app');dcSame(0,count($pShadow->events->rows),'shadow no event');
