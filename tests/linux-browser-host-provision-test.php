...
lbAssert(!str_contains($svc,'NoNewPrivileges=true'),'Snap Chromium requires snap-confine capability setup; NoNewPrivileges must not block browser startup.');
lbAssert(str_contains($svc,'ProtectSystem=full') && str_contains($svc,'PrivateTmp=true'),'Browser service must retain filesystem and temporary-directory hardening.');
