...
dorSame('SAFE_T_APPEAL',$newDate['action'],'a newer Amazon wait cannot cross the official internal appeal deadline');dorSame('APPEAL_DEADLINE_PREEMPTS_AMAZON_WAIT',$newDate['reason']??null,'later wait must preserve internal appeal rights');dorSame('2026-09-18 03:00:00',$newDate['next_action_at']??null,'new date remains recorded even when internal deadline preempts it');
$old=$wait;$old['occurred_at']='2026-08-01 12:00:00';$old['payload']['decision_text']='Aguarde ate 02/08/2026.';
$oldResult=$engine->nextAction($claim,[$changed,$old],$policy,new DateTimeImmutable('2026-09-10T13:00:00Z'));dorSame('SAFE_T_APPEAL',$oldResult['action'],'late-arriving older response must not replace the newer conflicting wait');dorSame($newDate['idempotency_key']??null,$oldResult['idempotency_key']??null,'same latest wait must keep one preemptive appeal idempotency key');
