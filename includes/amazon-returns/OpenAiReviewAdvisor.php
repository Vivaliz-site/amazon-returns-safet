...
final class SvAmazonOpenAiReviewAdvisor implements SvAmazonReviewAdvisor
{
    ...
    private const LANGUAGE_INSTRUCTION='Recommend one SAFE-T review action using only supplied structured facts. Do not invent dates or execute actions. Write rationale and every uncertainty in clear Brazilian Portuguese (pt-BR) for a non-technical operator. Keep enum values exactly as required by the schema.';
    ...
    {
        $schema=self::schema();$prompt=$this->prompt($context);$instruction=self::LANGUAGE_INSTRUCTION;
        ...
        return [
            ...
            ['key'=>$this->config->anthropicKey(),'model'=>$claudeModel,'request'=>['url'=>'https://api.anthropic.com/v1/messages','headers'=>['x-api-key'=>$this->config->anthropicKey(),'anthropic-version'=>'2023-06-01','Content-Type'=>'application/json'],'body'=>['model'=>$claudeModel,'max_tokens'=>1200,'system'=>$instruction,'messages'=>[['role'=>'user','content'=>$prompt]],'output_config'=>['format'=>['type'=>'json_schema','schema'=>$schema]]]],'parse'=>static function(array $body):mixed{$text=null;foreach(($body['content']??[]) as $c)if(($c['type']??'')==='text'){$text=$c['text']??null;break;}return self::decode($text);}],
            ['key'=>$this->config->geminiKey(),'model'=>$geminiModel,'request'=>['url'=>'https://generativelanguage.googleapis.com/v1beta/models/'.rawurlencode($geminiModel).':generateContent','headers'=>['x-goog-api-key'=>$this->config->geminiKey(),'Content-Type'=>'application/json'],'body'=>['contents'=>[['role'=>'user','parts'=>[['text'=>$instruction.'\n'.$prompt]]]],'generationConfig'=>['responseMimeType'=>'application/json','responseJsonSchema'=>$schema,'thinkingConfig'=>['thinkingLevel'=>'LOW']]]],'parse'=>static function(array $body):mixed{$text=$body['candidates'][0]['content']['parts'][0]['text']??null;return self::decode($text);}],
            ['key'=>$this->config->geminiKey(),'model'=>$geminiFallbackModel,'request'=>['url'=>'https://generativelanguage.googleapis.com/v1beta/models/'.rawurlencode($geminiFallbackModel).':generateContent','headers'=>['x-goog-api-key'=>$this->config->geminiKey(),'Content-Type'=>'application/json'],'body'=>['contents'=>[['role'=>'user','parts'=>[['text'=>$instruction.'\n'.$prompt]]]],'generationConfig'=>['responseMimeType'=>'application/json','responseJsonSchema'=>$schema,'thinkingConfig'=>['thinkingLevel'=>'LOW']]]],'parse'=>static function(array $body):mixed{$text=$body['candidates'][0]['content']['parts'][0]['text']??null;return self::decode($text);}],
        ];
    }
    ...
    private function input(array $c):array{return [['role'=>'system','content'=>[['type'=>'input_text','text'=>self::LANGUAGE_INSTRUCTION]]],['role'=>'user','content'=>[['type'=>'input_text','text'=>$this->prompt($c)]]]];}
