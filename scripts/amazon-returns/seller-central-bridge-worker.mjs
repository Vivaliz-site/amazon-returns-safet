...
async function selectSafeTSubreason(cdp, value) {
  ...
  for (let attempt=0; attempt<12; attempt++) {
    const selected=(await cdp.evaluate(`(()=>{const hints=${JSON.stringify(hints)};const expected=${JSON.stringify(expected)};const normalize=v=>String(v||'').normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').trim().toLowerCase();const docs=[];const visit=d=>{if(!d||docs.includes(d))return;docs.push(d);for(const frame of d.querySelectorAll('iframe')){try{visit(frame.contentDocument)}catch{}}};visit(document);const candidates=[];for(const d of docs){for(const host of d.querySelectorAll('kat-dropdown')){const roots=[host.shadowRoot,host].filter(Boolean);const options=[];const seen=new Set();for(const root of roots){for(const option of root.querySelectorAll('kat-option')){if(seen.has(option))continue;seen.add(option);options.push(option)}}const exact=options.filter(option=>String(option.getAttribute('value')||'').trim()===expected);if(exact.length>1)return false;if(exact.length!==1)continue;const meta=normalize([host.getAttribute('placeholder'),host.getAttribute('label'),host.getAttribute('aria-label'),host.getAttribute('name'),host.id,host.className].filter(Boolean).join(' '));candidates.push({option:exact[0],hinted:hints.some(h=>meta.includes(h))})}}let chosen=null;if(candidates.length===1)chosen=candidates[0];else{const hinted=candidates.filter(candidate=>candidate.hinted);if(hinted.length!==1)return false;chosen=hinted[0]}chosen.option.click();return true})()`))===true;
