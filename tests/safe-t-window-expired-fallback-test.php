...
if(($decision['action']??null)!=='SELLER_SUPPORT_OPEN'){fwrite(STDERR,'D+89 from refund_at expected SELLER_SUPPORT_OPEN, got '.json_encode($decision).PHP_EOL);exit(1);}
...
if(($decisionD90['action']??null)!=='WAIT'){fwrite(STDERR,'D+90 from refund_at must be outside active scope, got '.json_encode($decisionD90).PHP_EOL);exit(1);}
if(($decisionD90['reason']??null)!=='RECOVERY_WINDOW_EXPIRED'){fwrite(STDERR,'D+90 unexpected reason '.json_encode($decisionD90).PHP_EOL);exit(1);}
$noRefund=$case;$noRefund['refund_at']=null;
if(SvAmazonRecoveryWindow::expired($noRefund,$d90)!==false){fwrite(STDERR,'Case without refund_at must remain investigable, not expired'.PHP_EOL);exit(1);}
