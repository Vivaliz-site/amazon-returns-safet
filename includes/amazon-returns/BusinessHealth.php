...
{
    /** @param array<string,array{ready?:bool,missing?:array}> $readiness @param array<string,bool> $writeFlags */
    public static function evaluate(bool $enabled,string $mode,array $readiness,array $writeFlags,array $browserLiveness): array
    {
        ...
        foreach(['sp_api','gmail','seller_central_bridge'] as $dependency){
            if(($readiness[$dependency]['ready']??false)!==true){
                $blockers[]='READINESS_'.strtoupper($dependency);
            }
        }
        ...
        foreach([
            ...
            'SELLER_SUPPORT_OPEN','SELLER_SUPPORT_UPDATE','ERP_SALES_RETURN_CREATE',
        ] as $action){
            if(($writeFlags[$action]??false)!==true)$blockers[]='WRITE_GATE_'.$action.'_DISABLED';
        }
        return ['status'=>$blockers===[]?'OK':'DEGRADED','blockers'=>array_values(array_unique($blockers))];
    }
}
