...
$p3=new RsP;$s3=new SvAmazonReviewService($p3,new SvAmazonDecisionCoordinator(new SvAmazonSafeTDecisionEngine(),$p3,null));$literal=$d;$literal['parameters']=['date'=>'2026-09-10','date_binding'=>'PROMISED_DATE'];rsThrows(fn()=>$s3->preview(1,$literal),'literal reusable date rejected');
$p4=new RsP;$s4=new SvAmazonReviewService($p4,new SvAmazonDecisionCoordinator(new SvAmazonSafeTDecisionEngine(),$p4,null));$wait=$d;$wait['decision_mode']='WAIT';$wait['final_action']='WAIT';$wait['parameters']=['date_binding'=>'NONE'];rsThrows(fn()=>$s4->preview(1,$wait),'reusable WAIT without a date binding rejected');
