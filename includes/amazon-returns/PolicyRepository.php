...
final class SvAmazonReturnPolicyRepository
{
        ...
        try{
            ...
            foreach($definitions as $definition){
                ...
                if(!is_array($existing)){
                    $this->insertOperationalVersion($row);
                    $stmt->execute($params);
                    $existing=$stmt->fetch(PDO::FETCH_ASSOC);
                    if(!is_array($existing))throw new RuntimeException('Operational version insertion was not visible.');
                }
                if(is_array($existing)){
    ...
    private function insertOperationalVersion(array $row): void
    {
        $stmt=$this->db->prepare('INSERT INTO amazon_return_policies '
        ...
        if(!$stmt instanceof PDOStatement)throw new RuntimeException('Operational version insertion failed.');
        $parameters=[':tenant_id'=>$this->context->tenantId()];
        foreach($row as $key=>$value)$parameters[':'.$key]=$value;
